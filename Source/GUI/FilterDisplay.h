...
public:
    ...
    public:
        FreqBand(int _harm) :
            params(A4_FREQ, RESONANCE_DEFAULT, TIMBRE_DEFAULT, CURVE_DEFAULT, SPREAD_DEFAULT, RETUNE_DEFAULT),
            harm(_harm)
        {
        }
        ...
        void updateParams(float freq, float resonance, float timbre, float curve, float spread, CombProcessor::FreqOutOfBoundsMode mode)
        {
            params.freq = freq;
        ...
        {
            freq = harm ? params.freq * pow(float(harm + 1), params.spread) : params.freq;
    ...
    FilterDisplay(TheFineToothAudioProcessor& p) : audioProcessor(p)
    {
    ...
    {
        std::fill(freqBands.begin(), freqBands.end(), nullptr);
    }
    ...
    void updateAll()
    {
        ...
        auto freq = audioProcessor.getFreq();
        
        for (int harm = 0; harm < MAX_NUM_FILTERS; ++harm)
        {
            freqBands[harm]->updateParams(freq, settings.resonance, settings.timbre, settings.curve, settings.spread, CombProcessor::FreqOutOfBoundsMode::Ignore);
        }
    }
